...
function recompute(plan, comp) {
  ...
  scenes.forEach((s, idx) => {
    ...
    const content = assetTruthy(s.asset) || s.type !== "coldopen" || authoredContent(s, idx);
...
export function verifyFilm(brief, plan, comp) {
  ...
  try {
    ...
    if (m.continuousShotSeconds < 6 && bestRun < 3) {
      out.issues.push(`longest continuous shot ${r3(m.continuousShotSeconds)}s (<6s)`);
    }
    if (!scenes.some((s) => Number(s.duration) >= 5)) {
